		...
		if (input == "ADD") {
			...
			while (keepadding == true) {
				...
				if (type == "random") {
					...
					int count = 0;
					while (count <= amount) {
						addStudent(generator->GetRandomStudent(ids), hashSize, hashTable, generator);
						count++;
					}
					...
					if (invalidid == false) {
						addStudent(student, hashSize, hashTable, generator);
					}
					else {
...
void addStudent(Student* newstudent, int hashSize, Student** hashTable, Generator* generator) {
	...
	// otherwis loops through key's linked list and add to end
		...
		if (nodes == 1) {
			rehash(hashTable, hashSize, generator);
		}
	}
}
...
void rehash(Student** &hashTable, int &hashSize, Generator* generator){
	...
	// hashSize doubles 
	 ...
	 for (int i = 0; i < (hashSize/2); i++) {
		 if (temp[i] != NULL) {
			 addStudent(temp[i], hashSize, hashTable, generator);
		 }
	 }
	 // delete old table
	 delete[] temp;
